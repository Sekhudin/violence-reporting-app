...
const TextName = ({ text, className, }: { text: string; className?: string }) => (
  <p className={cn(`flex justify-start text-sm sm:text-base font-semibold text-center capitalize`,
    className)}>
    {FunStr.abbrevName(text, 3)}
  </p>);

const TextEmail = ({ text, className, }: { text: string; className?: string }) => (
  <p className={cn(`flex justify-start text-sm sm:text-base font-light text-center capitalize`,
    className)}>
...
export const CardUserContact = React.forwardRef<HTMLDivElement, CardUserContactProps>(({
  ...
  return (
    <Card ref={ref}
      ...
      <CardContent className="flex items-center text-base space-x-4 p-0">
        ...
        <div className="h-20 grow flex flex-col justify-between">
          <div>
            ...
            <TextEmail text={v.email} className="lowercase truncate" />
          </div>
