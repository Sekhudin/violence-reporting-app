...
} from 'src/component/atoms/pdf/document';
import { Case } from 'src/service/case/case.service';
import { cn, FunDate, FunStr } from 'src/util';

export type CaseValues = {} & Case.Expose;

export type PdfCaseProps = {
  values: CaseValues;
  className?: string;
}
...
export const PdfCase = React.forwardRef<HTMLDivElement, PdfCaseProps>(({
  values: v,
  className
}, ref) => {
    ...
    <PdfDocument ref={ref} className={cn("relative", className)}>
      ...
      <PdfBody className='space-y-6'>
        ...
        <PdfSection
          ...
          <PdfField label="Waktu Kejadian" value={FunDate.ISOtoLocal(v.date_incident as unknown as string)} />
